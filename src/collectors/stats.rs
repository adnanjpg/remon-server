        ...
        *state.stats_latest.write().await = Some(bundle);
        ...
        if let Err(e) = metrics_repo
            .insert_raw_tick(
                ...
                &network,
                pressure.as_deref(),
                // Only on refresh ticks: persisting the cached reading would
                // stamp a measurement that was never taken. The broadcast and
                // SSE primer above still carry it every tick.
